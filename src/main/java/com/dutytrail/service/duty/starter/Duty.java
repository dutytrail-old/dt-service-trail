...
public class Duty {
    ...
    private static void configureHystrix() {
        for (Method method : DutyService.class.getMethods()) {
            ConfigurationManager.getConfigInstance().setProperty(String.format("hystrix.command.%s.execution.isolation.thread.timeoutInMilliseconds", method.getName()), 15000);
        }
    }
